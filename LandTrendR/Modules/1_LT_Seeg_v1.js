 ...
 * MODIFIED 2/2023 @author Edriano Souza (IPAM) | Barbara Zimbres (IPAM)
 * 
...
var getSRcollection = function(year, startDay, endDay, sensor, aoi, maskThese, exclude) {
  ...
  srCollection = srCollection.map(function(img) {
    ...
    // ## Add Mapbiomas Collection 7 v2
    
    // var aoi_cerrado = ee.FeatureCollection('users/barbarazimbres/Cerrado_2019');
    
    var forCol = ee.Image('projects/mapbiomas-workspace/public/collection7/mapbiomas_collection70_integration_v2');

    //var imgFor = forCol.toBands();
    var forestimage = forCol.select('classification_2019').clip(aoi);
    
    
    // New Advances
    
    // ## Create and filter natural classes 
    // ### create rule on persistence in 1 year in post processing of natural classes  
    ...
    if(maskThese.length !== 0){
      ...
      for(var i in maskThese){
        ...
        if(maskThese[i] == 'nonforest'){mask = mask.mask(forestimage)} // PETER ADD
      }
